...
public:
    ...
    DataType TranslateType(const char *name)
    {
        ...
        // FIXME: Missing types added for primitive type StructuredBuffers, but not yet rest of decompiler:
#define DT(x) do { if (!strcmp(name, #x)) return DT_##x; \
} while (0)
...
#undef DT
    ...
    void ReadResourceBindings(const char *c, size_t size)
    {
        ...
        while (pos < size)
        {
            ...
            {
                char    *escapePos = strchr(name, '['); if (escapePos)
                    *escapePos = '_';

                escapePos = strchr(name, ']'); if (escapePos)
                    *escapePos = '_';
                    ...
                    for (int i = 0; i < numElements; ++i)
                    {
                        ...
                        switch (e.bt)
                        {
                            ...
                            case DT_float2x4:  // Not positive this is right
                                ...
                                mCBufferData[offsetPos] = e; if (structLevel >= 0)
        ...
        else
        {
            ...
            if (strPos)
            {
                    ...
                    ((i->second.bt == DT_float3 || i->second.bt == DT_uint3 || i->second.bt == DT_int3 || i->second.bt == DT_bool3) && strrchr(right2, '.')[1] == 'w'))
                {
                    ...
                    if (i->second.bt == DT_float || i->second.bt == DT_uint || i->second.bt == DT_int || i->second.bt == DT_bool)
                        skip = 1;
                    else if (i->second.bt == DT_float2 || i->second.bt == DT_uint2 || i->second.bt == DT_int2 || i->second.bt == DT_bool2)
                        skip = 2;
                    else if (i->second.bt == DT_float3 || i->second.bt == DT_uint3 || i->second.bt == DT_int3 || i->second.bt == DT_bool3)
                        skip = 3;

                    char    *dotPos     = strrchr(right2, '.');
                    int     lowOffset   = dotPos[1] - 'x'; if (dotPos[1] == 'w')
                        lowOffset = 3;
                    ...
                    if (lowOffset >= skip)
                    {
                        ...
                        {
                            lowOffset = dotPos[cpos] - 'x'; if (dotPos[cpos] == 'w')
                                lowOffset = 3;
            ...
            else
            {
                switch (instr->eOpcode)
                {
                    ...
                    case OPCODE_MOV:
                        ...
                        if (op1[0] == 'o')
                        {
                            char    *dotPos = strchr(op1, '.'); if (dotPos)
                                *dotPos = 0;
                    ...
                    case OPCODE_SWAPC:
                    {
                        ...
                        int     idx     = 0;
                        char    *pop1   = strrchr(op1, '.'); *pop1 = 0;
                        char    *pop2   = strrchr(op2, '.'); if (pop2)
                            *pop2 = 0;

                        char    *pop3 = strrchr(op3, '.'); if (pop3)
                            *pop3 = 0;
                        ...
                        while (*++pop1)
                        {
                            ...
                            if (pop2)
                                sprintf(op7, "%s.%c", op2, *++pop2);
                            else
                                sprintf(op7, "%s", op2);

                            if (pop3)
                                sprintf(op8, "%s.%c", op3, *++pop3);
                            else
                                sprintf(op8, "%s", op3);
